...
検索対象日は JST の当日固定です。画面では開始時刻・終了時刻・ホスト名・プログラム名・メッセージを指定し、条件に一致したログのうち最新 50 件を表示します。
...
GET でも同じパラメータをクエリ文字列で渡せます。

```bash
curl "http://localhost:5004/api/logs?log_type=syslog&message=timeout"
```
...
- JST の時刻表示変換（エポックミリ秒・文字列タイムスタンプ）
- JST 当日の時刻範囲を Trino SQL に変換する処理
- SQL のシングルクォート・識別子クォート・LIKE エスケープの安全性
- `syslog_events` / `authlog_events` を対象にした SQL 生成（UNION ALL 含む）
- `POST /` による画面検索（セッション経由の PRG パターン）
- `POST /api/logs` による JSON API 検索
- 検索結果の表示用フィールド (`display_time` 等) の作成
